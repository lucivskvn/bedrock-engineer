...
export class VectorStoreManager {
  ...
  private async getStore(): Promise<FaissStore> {
    if (!this.store) {
        // Workaround: FaissStore must be initialized with at least one document.
        // This creates a temporary store if one wasn't loaded from disk and `addDocuments` hasn't been called yet.
        // This allows `similaritySearch` to be called on a conceptually "empty" store without errors.
        console.log("Creating a temporary, empty vector store for querying.");
