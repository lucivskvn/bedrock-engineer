...
export class PersistentMemoryService {
  ...
  async retrieveContextualMemories(query: string, sessionId?: string, k?: number): Promise<Document[]> {
    ...
    // TODO: Implement session-based memory filtering.
    // FAISS does not support metadata filtering out-of-the-box. An enhancement could be:
    // 1. Use a different vector store that supports metadata filters (e.g., Chroma).
    // 2. Retrieve a larger number of documents (k * n) and filter them by sessionId in the application logic.
    // 3. Maintain separate FAISS indexes per session.
    // For now, all memories are retrieved from the global store.
    if (sessionId) {
      console.warn("Session ID filtering is not yet implemented. Retrieving from global memory.");
    }
